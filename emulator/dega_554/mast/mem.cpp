...
#ifdef TRACE
static FILE* mem_trace = fopen("chi_mem_trace.txt", "w");
static FILE* io_trace = fopen("chi_io_trace.txt", "w");
#endif
...
#ifdef TRACE
	fprintf(mem_trace, "%.8x, PC %.4x:  ", inst_count, Doze.o_pc );
	fprintf(mem_trace, "addr: %.4x, data %.2x\t<Load>\n", a, d);
#endif
...
#ifdef TRACE
	fprintf(mem_trace, "%.8x, PC %.4x:  ", inst_count, Doze.o_pc );
	fprintf(mem_trace, "addr: %.4x, data %.2x\t<Store>\n", a, d);
#endif
...
#ifdef TRACE
	fprintf(io_trace, "%.8x, PC %.4x:  ", inst_count, Doze.o_pc );
	fprintf(io_trace, "addr: 00%.2x, data %.2x\t<In>\n", (unsigned char)(a&0x00ff), d);
#endif
...
//	if ((a == 0xdd) || (a == 0xf2))
//	ofstream outfile("..\\port_trace.txt", ios::app);
//	if ((a == 0xbe) || (a == 0xbf) || (a == 0x7e) || (a == 0x7f))
//	{ 	outfile << hex << inst_count << '\t' << hex << a << '\t' << hex <<
...
#ifdef TRACE
	fprintf(io_trace, "%.8x, PC %.4x:  ", inst_count, Doze.o_pc );
	fprintf(io_trace, "addr: 00%.2x, data %.2x\t<Out>\n", (unsigned char)(a&0x00ff), d);
#endif
